...
//指令下发:上报控制
int report_cotrol_DecodeCmdDown (char* source, report_cotrol* dest)
{
	char* index = source;
	int srcStrLen = strlen(source);
	int len = 4;
	...
	memset(dest, 0, sizeof(report_cotrol));
...
AepCmdData decodeCmdDownFromStr(char* source)
{
	...
	{
		result.serviceIdentifier = "report_cotrol";
		result.data = malloc(sizeof(report_cotrol));
		memset(result.data, 0, sizeof(report_cotrol));
		result.code = report_cotrol_DecodeCmdDown(index, (report_cotrol*)result.data);
	}
